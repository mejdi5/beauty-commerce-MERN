...
router.post('/register', nameValidation, emailValidation, passwordValidation , validator, async (req, res) => {
    ...
    try {
        ...
        const url = `https://beauty-commerce.herokuapp.com/verify/${savedUser._id}/${newToken.token}`;
		await sendEmail(savedUser.email, "Account Activation", url);
        res.status(201).json({ msg: 'Registred with success, Check you email to activate your account', savedUser, token: newToken, url });
    } catch (error) {
...
router.post('/login', validator, async (req, res) => {
    try {
        ...
        if (user && !user.verified) {
            ...
            //send activation email
            const url = `https://beauty-commerce.herokuapp.com/verify/${user._id}/${token.token}`;
